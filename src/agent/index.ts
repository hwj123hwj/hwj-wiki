...
export type OpenWikiAgentOptions = {
  command: OpenWikiCommand;
  connectorToolProfile?: OpenWikiRunOptions["connectorToolProfile"];
  cwd: string;
  ...
  return createDeepAgent({
    ...
    middleware:
      options.command === "chat"
        ? []
        : [
            ...(options.connectorToolProfile !== "none"
...
async function runOpenWikiAgentCore(
  ...
  const agent = inStageSync(
    ...
    () =>
      createOpenWikiAgentGraph({
        command,
        connectorToolProfile: options.connectorToolProfile,
        cwd,
  ...
  const metadataWritten = await inStage("finalize", async () => {
    ...
    if (options.suppressRunMetadata) return false;
