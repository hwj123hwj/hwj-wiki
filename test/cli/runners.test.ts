...
vi.mock("../../src/personalization/run.ts", () => ({
  runPersonalizedOpenWikiAgent: vi.fn(),
}));
...
import { runPersonalizedOpenWikiAgent } from "../../src/personalization/run.ts";
import {
...
describe("runPrintCommand", () => {
  test("runs the agent, prints collected text, and exits 0", async () => {
    vi.mocked(runPersonalizedOpenWikiAgent).mockImplementation(
      (
        _command: unknown,
        _cwd: unknown,
  ...
  test("pulls code-mode connectors before the agent in code mode", async () => {
    ...
    vi.mocked(runPersonalizedOpenWikiAgent).mockResolvedValue(
      undefined as never,
    );
    ...
    const agentArgs = vi.mocked(runPersonalizedOpenWikiAgent).mock.calls[0];
  ...
  test("prints the how-to-fix panel on an auth failure and exits 1", async () => {
    ...
    vi.mocked(runPersonalizedOpenWikiAgent).mockRejectedValue({
      status: 401,
