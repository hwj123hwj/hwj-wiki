...
11. `src/connectors/` contains the connector registry, MCP client/runtime, a shared resilient HTTP helper (`http.ts`), source-specific ingestion modules (git-repo, internal, gmail, hackernews, slack, web-search, x), and tool definitions exposed to the agent.
