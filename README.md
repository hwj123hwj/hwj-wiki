...
故障时状态为 `partial`，重新运行会从最后成功批次继续。
交互界面会明确显示 `Partial`；`--print` 在 partial 时返回退出码 `2`，方便脚本
区分“本轮安全停下、仍需续跑”和真正的 complete。

个人模式先把脱敏历史提取成带 `stableKey`、`sourceRefs`、可信度和时效标记的
结构化候选，再交给原 OpenWiki Agent 合并与写作。若兼容模型不能驱动完整
Agent 工具协议，安全降级也只消费结构化候选，并执行同一套 OKF、索引、断链、
去重、来源和敏感信息检查；降级页面进入独立复核队列，不会阻塞后续原始批次，
复核队列未清空时也绝不会误报 `complete`。
