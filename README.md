...
个人历史通过本地待处理队列分批整理：每轮最多约 100 条或 100KB，失败批次
不会丢失，重新运行 `openwiki personal --update` 会从待处理批次继续。
若本地兼容模型无法通过完整 Agent 协议调用工具，个人模式会自动切换到受限的
安全生成通道；它只能读取已脱敏批次并写入个人 Wiki 允许的目录。
